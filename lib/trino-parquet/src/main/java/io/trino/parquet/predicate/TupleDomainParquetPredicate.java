        ...
        implements Predicate
{
            ...
            DateTimeZone timeZone)
    {
        ...
        if (type.equals(BOOLEAN) && min instanceof Boolean) {
        ...
        if ((type.equals(BIGINT) || type.equals(TINYINT) || type.equals(SMALLINT) || type.equals(INTEGER) || type.equals(DATE)) && min instanceof Number) {
            ParquetLongStatistics parquetLongStatistics = toParquetLongStatistics(min, max);
            if (isStatisticsOverflow(type, parquetLongStatistics)) {
                return Domain.create(ValueSet.all(type), hasNullValue);
            }
            return createDomain(type, hasNullValue, parquetLongStatistics);
        }
        ...
        if (type instanceof DecimalType) {
            ...
            if (decimalType.isShort()) {
                ...
                return createDomain(type, hasNullValue, parquetSliceStatistics);
            }
        }

        if (type.equals(REAL) && min instanceof Float) {
            ...
            ParquetLongStatistics parquetStatistics = new ParquetLongStatistics(
                    (long) floatToRawIntBits(minValue),
                    (long) floatToRawIntBits(maxValue));
            return createDomain(type, hasNullValue, parquetStatistics);
        }

        if (type.equals(DOUBLE) && min instanceof Double) {
            ...
            ParquetDoubleStatistics parquetDoubleStatistics = new ParquetDoubleStatistics(minValue, maxValue);
            return createDomain(type, hasNullValue, parquetDoubleStatistics);
        }

        if (type instanceof VarcharType && min instanceof Binary) {
            Binary minValue = (Binary) min;
            Binary maxValue = (Binary) max;
        ...
        if (type instanceof TimestampType && min instanceof Binary) {
